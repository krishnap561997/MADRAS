...
for i=1:numel(ACH_list)
    ...
    n_wm = get_nwm(ACH,H,V,eta_ef,lam_v,activity,max(dT2_list)+T0+dT1);
    % Get distance specific concentration
    n_d = get_nd(n_wm,ACH,eta_ef,d);

    for j=1:numel(dT2_list)
...
dT2_list = dT2_list./60;
T0 = T0/60;
dT1 = dT1/60;

fname = strcat(['CumExp_dist_',num2str(d),...
    'm_T0_',num2str(T0),'min_dT1_',num2str(dT1),'min.mat']);
save(fullfile(fpath,fname),'D_d','d','dT2_list','dT1','T0','ACH_list','V','H');
