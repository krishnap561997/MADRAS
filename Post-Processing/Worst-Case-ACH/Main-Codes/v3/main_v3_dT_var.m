...
for i=1:numel(ACH_list)
    ...
    for j=1:numel(dT1_list)
        ...
        ctr = 1;
        while dT_pre < dT
            dT_post = round(dT - dT_pre);
            disp(strcat(['ACH: ',num2str(ACH),'. dT_pre: ',num2str(dT_pre),' sec. dT_post = ',num2str(dT_post)]));
            tmp(ctr) = get_exposure(n_d,ACS,V,dT_pre,T0,dT_post);
            dT_pre = dT_pre + 0.1*60;
        end 
        D_d_max(i,j) = max(tmp);
        D_d_min(i,j) = min(tmp);
        D_d_mean(i,j) = mean(tmp);
        D_d_var(i,j) = std(tmp);
        clear tmp
    end
    toc
...
save(fullfile(fpath,fname),'D_d_max','D_d_min','D_d_mean','D_d_var',...
    'd','dT1_list','dT','T0','ACH_list','V','H');
