...
path =  '/blue/bala/krishnap.kalivel/COVID/Sou2Snk_Spheres/dist_1m/room_29/Data'

addpath(genpath(path))

fpath = '/blue/bala/krishnap.kalivel/COVID/Sou2Snk_Spheres/dist_1m/room_29/Figures'
 

fontsize = 20;  nxt = 25;   nyt = 8;    nzt = 25; time_limit = 700;
d = 0.4;
V_sink = (1/48)*pi*(d^3);
...
%    ylabel('$\overline{N}$/$\forall_{sink}$','Interpreter','latex')
%    xlabel('t (s)','Interpreter','latex')
%    set(gca,'FontSize',18)
...
%    xlabel('t (s)','Interpreter','latex')
%    set(gca,'FontSize',18)
%    title({strcat('dp: ',num2str(unique_dp(i)),'$\mu$m')}, 'FontSize', 18, 'Interpreter', 'latex')
%    set(gcf, 'Units', 'Inches', 'Position', [0, 0, 16, 9], 'PaperUnits', 'Inches', 'PaperSize', [16 9])
%
...
%    figure(i);
%    X = std(Nhat_end(i,1,:));
%    Y = skewness(Nhat_end(i,1,:));
%    histogram(Nhat_end(i,1,:),50);
%    ylabel('PDF')
%    xlabel('$\widehat{N}$/$/forall_{sink}$','Interpreter','latex')
%    set(gca,'FontSize',18)
%    title({strcat('dp: ',num2str(unique_dp(i)),' $\mu$m',' $\sigma$ = ',num2str(X),' Sk = ',num2str(Y))}, 'FontSize', 18, 'Interpreter', 'latex')
%    set(gcf, 'Units', 'Inches', 'Position', [0, 0, 16, 9], 'PaperUnits', 'Inches', 'PaperSize', [16 9])
%    name = strcat('Nhat_PDF_',num2str(unique_dp(i)),'micron.pdf');
%    print('-dpdf','-r300',fullfile(fpath, name))
%    name = strcat('Nhat_PDF_',num2str(unique_dp(i)),'micron.fig');
%    saveas(gcf,fullfile(fpath, name))
%    name = strcat('Nhat_PDF_',num2str(unique_dp(i)),'micron.png');
%    print('-dpng','-r300',fullfile(fpath, name))
%end
%
%close all;
...
%end
%set(gcf, 'Units', 'Inches', 'Position', [0, 0, 16, 9], 'PaperUnits', 'Inches', 'PaperSize', [16 9])
%     name = strcat('Nhat_PDF.pdf');
%     print('-dpdf','-r300',fullfile(fpath, name))
%     name = strcat('Nhat_PDF.fig');
%     saveas(gcf,fullfile(fpath, name))
%     name = strcat('Nhat_PDF.png');
%     print('-dpng','-r300',fullfile(fpath, name))

text = 'FigureData';
save(fullfile(fpath,text),'Nhat_end','Nexit_mean', 'Nexit_std', 'Nhat_mean', 'Nhat_std','sinks_loc_c','source_loc_c');
