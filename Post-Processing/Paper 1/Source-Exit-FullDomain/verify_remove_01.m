...
function [id,dp,time,exit_surf,exit_loc] = verify_remove_01(id,dp,time,exit_surf,exit_loc,nfile)
...
idx2 = find(toto2==0);
if (sum(numel(idx2))~=0)
    disp([num2str(sum(numel(idx2))),' unaccounted particles have been removed'])
end
