...
Nhat_cum = zeros(size(N_cum));
t = 0:(size(N_cum,3)-1);
for i=2:size(N_cum,3)
    Nhat_cum(:,:,i) = sum(N_cum(:,:,1:i),3)./t(i);
end

%% Process Data
N2_cum = reshape(mean(Nhat_cum,2),[size(Nhat_cum,1),size(Nhat_cum,3)]) ;
N1_cum = reshape(mean(Nhat_cum,1),[size(Nhat_cum,2),size(Nhat_cum,3)]) ;
N0_cum = reshape(Nhat_cum, [size(Nhat_cum,1).*size(Nhat_cum,2),size(Nhat_cum,3)]);

N1_std_v2 = reshape(std(Nhat_cum,1,2),[size(Nhat_cum,1),size(Nhat_cum,3)]);
N2_std_v2 = reshape(std(Nhat_cum,1,1),[size(Nhat_cum,2),size(Nhat_cum,3)]);
...
%% Save Data
fname = strcat(['StatsData_Cum_Dist_',num2str(dist),'m.mat']);
