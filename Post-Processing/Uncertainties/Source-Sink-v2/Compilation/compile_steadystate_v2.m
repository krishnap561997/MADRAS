	    ...
	    time_min = min(time_limit);
            ...
            for l1=1:size(Nhat_cum,1)
                for l2 = 1:size(Nhat_cum,2)
                    tmp = Nhat_cum{l1,l2};
                    Nhat_ss_cum(ctr,l2) = tmp(time_min+1)./Ntot_cum(l1,l2);
		    Ntot_c(ctr,l2) = Ntot_cum(l1,l2);
...
text = strcat('SteadyStateData_Dist_',num2str(dist),'m_v2.mat');
save(fullfile(spath,text),'Nhat_ss_cum','Ntot_c','sinks_loc_c','source_loc_c','time_min');
disp('Data Saved');
